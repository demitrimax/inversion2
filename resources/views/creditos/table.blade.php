    ...
    @foreach($creditos as $creditos)
        <tr>
            ...
            <td>
                ...
                <div class='btn-group'>
                    <a href="{!! route('creditos.show', [$creditos->id]) !!}" class='btn btn-info'><i class="far fa-eye"></i></a>
                    @can('creditos-edit')
                    <a href="{!! route('creditos.edit', [$creditos->id]) !!}" class='btn btn-primary'><i class="far fa-edit"></i></a>
                    @endcan
                    @can('creditos-delete')
                    {!! Form::button('<i class="far fa-trash-alt"></i>', ['type' => 'button', 'class' => 'btn btn-danger', 'onclick' => "ConfirmDelete($creditos->id)"]) !!}
                    @endcan
                </div>
                {!! Form::close() !!}
            </td>
