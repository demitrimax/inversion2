            ...
            @foreach($clasifica->subcategorias as $subcategoria)
                <tr>
                    ...
                    <td>
                        ...
                        <div class='btn-group'>
                            <a href="{!! route('subclasificas.show', [$subcategoria->id]) !!}" class='btn btn-info btn-xs'><i class="fa fa-eye"></i></a>
                            @can('subclasificas-edit')
                            <a href="{!! route('subclasificas.edit', [$subcategoria->id]) !!}" class='btn btn-primary btn-xs'><i class="fa fa-pencil"></i></a>
                            @endcan
                            @can('subclasificas-delete')
                            {!! Form::button('<i class="fa fa-trash-o"></i>', ['type' => 'button', 'class' => 'btn btn-danger btn-xs', 'onclick' => "ConfirmDelete($subcategoria->id)"]) !!}
                            @endcan
                        </div>
                        {!! Form::close() !!}
                    </td>
